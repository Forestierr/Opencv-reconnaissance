...
class Main:
    ...
    def parse_frame(self, frame, results):
        ...
        if should_alert:
            img_h = frame.shape[0]
            img_w = frame.shape[1]
            cv2.putText(frame, "Too close", (int(img_w / 3), int(img_h / 2)), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 2)

            #Sound play with PyGame
            if not pygame.mixer.music.get_busy():
                if self.n_langue == 0 :
                    pygame.mixer.music.load("di_fr.mp3")
                    self.n_langue += 1
                elif self.n_langue == 1 :
                    pygame.mixer.music.load("di_de.mp3")
                    self.n_langue += 1
                elif self.n_langue == 2 :
                    pygame.mixer.music.load("di_it.mp3")
                    self.n_langue += 1
                elif self.n_langue == 3:
                    pygame.mixer.music.load("di_en.mp3")
                    self.n_langue = 0
...
class MainDebug(Main):
    ...
    def parse_frame(self, frame, results):
        ...
        test = cv2.resize(numpy_horizontal, (1920,1080), interpolation=cv2.INTER_AREA) ## Change for screen resolution !
